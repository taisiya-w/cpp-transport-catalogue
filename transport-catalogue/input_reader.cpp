...
namespace transport_catalogue {
    ...
    namespace io {
        ...
        void InputReader::ParseLine(std::string_view line) {
            auto command_description = transport_catalogue::detail::ParseCommandDescription(line);
        ...
        void InputReader::ApplyCommands([[maybe_unused]] transport_catalogue::core::TransportCatalogue& catalogue) const {
            ...
            for (const auto& c : commands_) {
                if (c.command == "Bus") {
                    std::vector<std::string_view> list = transport_catalogue::detail::ParseRoute(c.description);
                    std::deque<const transport_catalogue::core::Stop*> stops;
                    for (auto s : list) {
                        auto it = catalogue.FindStop(s);
                        if (it != nullptr) {
                            stops.push_back(it);
                        }
                    }
